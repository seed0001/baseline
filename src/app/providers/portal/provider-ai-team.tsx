...
export function ProviderAiTeam({
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold text-slate-950">Who do you want to work with?</h2>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">
          {personas.map((item) => (
            <button
              ...
              className={`rounded-2xl border p-3 text-left transition ${selectedKey === item.key ? "border-teal-600 bg-teal-50 ring-1 ring-teal-600" : "border-slate-200 hover:border-slate-300"}`}
            >
              <span className="text-xl">{item.key === "manager" ? "◎" : item.key === "finance" ? "$" : item.key === "marketing" ? "◇" : "↗"}</span>
              <p className="mt-2 text-sm font-semibold text-slate-900">{item.displayName}</p>
      ...
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-teal-900 via-teal-800 to-cyan-700 text-white shadow-lg">
        <div className="flex items-start justify-between gap-3 border-b border-white/15 p-5 sm:px-8">
          <div>
            <p className="font-semibold">{persona.displayName}</p>
            <p className="mt-1 text-xs text-teal-100">{persona.title}</p>
          </div>
          <div className="flex rounded-xl bg-black/15 p-1 text-xs font-semibold">
            <button onClick={() => setMode("chat")} className={`rounded-lg px-3 py-2 ${mode === "chat" ? "bg-white text-teal-900" : "text-teal-50"}`}>Conversation</button>
            <button onClick={() => setMode("build")} className={`rounded-lg px-3 py-2 ${mode === "build" ? "bg-white text-teal-900" : "text-teal-50"}`}>Build service</button>
          </div>
        </div>
        <div className="max-h-96 min-h-56 space-y-3 overflow-y-auto bg-slate-50 p-4 sm:p-6">
          {messages.length === 0 && (
          ...
          {messages.map((message, index) => (
            <div key={`${index}-${message.content.slice(0, 12)}`} className={message.role === "user" ? "flex justify-end" : "flex justify-start"}>
              <div className={`max-w-[88%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm ${message.role === "user" ? "rounded-br-sm bg-teal-700 text-white" : "rounded-bl-sm bg-white text-slate-700 shadow-sm"}`}>
                {message.content}
              </div>
            </div>
          ))}
          {pending && mode === "chat" && <div className="flex justify-start"><div className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-400 shadow-sm">Thinking…</div></div>}
        </div>
        <div className="bg-white p-2">
          <textarea
            ...
            className="min-h-20 w-full resize-none rounded-xl px-3 py-3 text-base text-slate-900 outline-none placeholder:text-slate-400"
          />
          <div className="flex items-center justify-between px-1 pb-1">
            <button onClick={listen} type="button" aria-label="Speak your message" className={`flex h-12 w-12 items-center justify-center rounded-full text-xl ${listening ? "animate-pulse bg-red-500 text-white" : "bg-slate-100 text-slate-700"}`}>●</button>
            <button onClick={submit} disabled={pending || !prompt.trim()} className="rounded-xl bg-teal-700 px-5 py-3 text-sm font-semibold text-white disabled:bg-slate-300">
              {pending ? "Working…" : mode === "chat" ? "Send" : "Create draft"}
            </button>
          </div>
        </div>
        <p className="px-5 py-3 text-xs text-teal-100 sm:px-8">Conversation is separate from actions. Nothing is published or sent without approval.</p>
      </section>
...
function PersonaEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4 sm:p-8">
      <div className="mx-auto max-w-xl rounded-3xl bg-white p-5 text-slate-900 shadow-2xl sm:p-8">
        ...
        <div className="mt-7 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-xl border border-slate-300 px-4 py-3 text-sm font-semibold">Cancel</button>
          <button disabled={pending || displayName.trim().length < 2} onClick={() => onSave({ key: persona.key, displayName, communicationStyle, customInstructions })} className="flex-1 rounded-xl bg-teal-700 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{pending ? "Saving…" : "Save persona"}</button>
        </div>
      </div>
    </div>
  );
